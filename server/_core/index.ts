...
import { runDueMonitoring } from "../monitoring";
import { registerApiV1 } from "../apiV1";
...
async function startServer() {
  ...
  app.post("/api/scheduled/monitor", async (req, res) => {
    try {
      ...
      return res.status(500).json({ error: error instanceof Error ? error.message : "monitoring failed", timestamp: new Date().toISOString() });
    }
  });
